        ...
        try:
            ...
            for profile_id, update_data in results.items():
                if update_data.get('success'):
                    ...
                    logger.error(f"Профиль {profile_id}: ошибка - {update_data.get('error')}")
            
            logger.info(f"Проверка завершена. Обработано профилей: {len(results)}")
        
        except Exception as e:
            logger.error(f"Ошибка в задаче проверки обновлений: {e}", exc_info=True)
