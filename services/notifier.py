...
class Notifier:
    ...
    async def notify_history_changes(self, user_id: int, username: str, entries: List[Dict]) -> bool:
        ...
        try:
            ...
            logger.info(f"Отправлено уведомление об истории для пользователя {user_id}")
            return True
    ...
    async def notify_online_status(self, user_id: int, username: str, 
        ...
        try:
            message = self._format_online_message(username, is_online, status_text)
            await self.bot.send_message(
                chat_id=user_id,
                text=message,
                parse_mode='HTML'
            )
            logger.info(f"Отправлено уведомление о статусе для пользователя {user_id}")
            return True
    ...
    async def process_profile_updates(self, profile_id: int, update_data: Dict):
        ...
        try:
            ...
            async with db.session_maker() as session:
                ...
                stmt = select(TrackedProfile).where(TrackedProfile.id == profile_id)
            ...
            logger.error(f"Ошибка обработки обновлений профиля {profile_id}: {e}")
